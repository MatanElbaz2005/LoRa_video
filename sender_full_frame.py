...
if __name__ == "__main__":
    ...
    try:
        ...
        zstd_compressor = zstd.ZstdCompressor(dict_data=zstd_dict)
        print(f"[Sender] Zstd dictionary loaded from {ZSTD_DICT_PATH}")
    except Exception as e:
        print(f"[Sender] Warning: Failed to load Zstd dictionary. Using default compression. Error: {e}")
    ...
    while True:
        ...
        frame_id_8bit = frame_id % 256

        mode_map = {4: 0, 6: 1, 8: 2, 10: 3}
        mode_counts = {4:0, 6:0, 8:0, 10:0}
        point_counts = {4:0, 6:0, 8:0, 10:0}
        
        converted_count = 0
        num_contours_valid = 0

        mini_headers_stream = BitStream()
        payloads_stream = BitStream()
        ...
        for pts, is_closed in contours_final_with_flags:
            ...
            contour_payload = BitStream()
            contour_payload.append(f'int:16={head[0, 0]}, int:16={head[0, 1]}')
            format_string = f'int:{mode}'
            for dx, dy in deltas:
                contour_payload.append(f'{format_string}={dx}, {format_string}={dy}')
            
            payload_len_bits = len(contour_payload)
            
            mini_headers_stream.append(f'uint:1={is_closed_bit}, uint:2={mode_bits}, uint:13={payload_len_bits}')
            payloads_stream.append(contour_payload)

        uncompressed_payload = BitStream()
        uncompressed_payload.append(f'uint:8={frame_id_8bit}')
        uncompressed_payload.append(f'uint:8={num_contours_valid}')
        uncompressed_payload.append(mini_headers_stream)
        uncompressed_payload.append(payloads_stream)
        
        uncompressed_bytes = uncompressed_payload.tobytes()
        compressed_bytes = zstd_compressor.compress(uncompressed_bytes)
        
        len_header = struct.pack(">H", len(compressed_bytes))
        
        final_packet = len_header + compressed_bytes
        total_bytes_sent = len(final_packet)
        
        sock.sendto(final_packet, (HOST, PORT))
        ...
        print(
            f"[SENDER-STATS] "
            f"frame={frame_id} (ID_8bit={frame_id_8bit}) "
            f"contours={num_contours} (sent={num_contours_valid}, skipped={full_A}, dropped_dist={dropped_count}, converted_lines={converted_count}) "
            f"modes (4/6/8/10)={mode_counts[4]}/{mode_counts[6]}/{mode_counts[8]}/{mode_counts[10]} "
            f"total_points={sum(point_counts.values())} "
            f"points (4/6/8/10)={point_counts[4]}/{point_counts[6]}/{point_counts[8]}/{point_counts[10]} "
            f"total_KB={(total_bytes_sent / 1024.0):.1f} "
            f"(uncompressed_KB={(len(uncompressed_bytes) / 1024.0):.1f})"
            )
